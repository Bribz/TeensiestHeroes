...
#pragma target 4.0
	...
	void geom(point v2g IN[1], inout TriangleStream<g2f> triStream)
	{
		...
		OUT.pos = UnityObjectToClipPos(v0 + (perpendicularAngle * (_GrassWidth * 0.5) * _GrassHeight));
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(0, 0);
		triStream.Append(OUT);

		OUT.pos = UnityObjectToClipPos(v1 + (perpendicularAngle * (_GrassWidth * 0.5) * _GrassHeight));
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(0, 1);
		triStream.Append(OUT);

		OUT.pos = UnityObjectToClipPos(v0 + perpendicularAngle * (_GrassWidth * -0.5));
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(1, 0);
		triStream.Append(OUT);

		OUT.pos = UnityObjectToClipPos(v1 + perpendicularAngle * (_GrassWidth * -0.5));
		OUT.norm = faceNormal;
		OUT.diffuseColor = color;
		OUT.uv = float2(1, 1);
		triStream.Append(OUT);
	...
	half4 frag(g2f IN) : COLOR
	{
		...
		float4 color = float4(IN.diffuseColor.rgb, 1.0);
		c *= color;
		clip(c.a - _Cutoff);
		return c;
	}
